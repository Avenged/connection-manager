        ...
        private static async Task<NamedPipeAsAppServiceConnection> BuildConnection(bool launchFullTrust)
        {
            try
            {
                if (launchFullTrust)
                {
                    ...
                    var host = WebAuthenticationBroker.GetCurrentApplicationCallbackUri().Host.ToUpper();
                    ApplicationData.Current.LocalSettings.Values["PackageSid"] = host;

                    await FullTrustProcessLauncher.LaunchFullTrustProcessForCurrentAppAsync();
                }

                var connection = new NamedPipeAsAppServiceConnection();
                if (await connection.Connect(@"LOCAL\ConnectionManagerInteropService_ServerPipe", TimeSpan.FromSeconds(19)))
                {
                    return connection;
                }
                connection.Dispose();
